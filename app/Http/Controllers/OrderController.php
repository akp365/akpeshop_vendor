...
class OrderController extends Controller
{
    ...
    public function view(Request $request)
    {
        ...
        $ordersWithDetails = $orders->map(function ($order) {
            ...
            $ordered_currency = $order->currency;
            $currency_rate = Currency::where('title', $ordered_currency)->first();
            ...
            // return $order;
            $order->final_invoice_value = (($order->invoice_value + $order->total_cod_fees + $order->seller_total_shipping_fee) - $order->total_discount) * $currency_rate->usd_conversion_rate;
            ...
            $order->earnings = round(($order->final_invoice_value) - ($order->seller_payable_money_raw), 2);
    ...
    public function convertCurrency($currentCurrency, $amount, $targetCurrency)
    {
        ...
        // Convert string amount to float if it's not already
        $numericAmount = is_string($amount) ? floatval(str_replace(',', '', $amount)) : floatval($amount);

        // Convert amount to USD first, then to target currency
        $amountInUSD = $numericAmount / floatval($currentCurrencyRate->usd_conversion_rate);
        $convertedAmount = $amountInUSD * floatval($targetCurrencyRate->usd_conversion_rate);

        return [
            'original_currency' => $currentCurrency,
            'original_amount' => number_format($numericAmount, 2, '.', ','),
