...
class ProductController extends Controller
{
    public function addNewProduct(Request $reqeust)
    {
        ...
        $currencyList = Currency::latest()->get();
        ...
        // $nestedCategories = $groupedCategories[0]->map(function ($parent) use ($groupedCategories) {
        //     $parent->subcategories = $groupedCategories[$parent->id] ?? [];
        //     return $parent;
        // });
        ...
        //COMPACT
        $data = compact('countryList', 'categoryList','vendorDetails','currencyList');
    ...
    public function productList(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            $finalData = Product::where('seller_id', Auth::id())->get();
            ...
            return DataTables::of($finalData)
                ...
                ->addColumn('name', function ($row) {
                    return '<p style="max-width: 500px; text-wrap: wrap;">'.$row->name.'</p>';
                })
                ...
                ->addColumn('action', function ($row) {
                    ...
                    {
                        if(in_array($row->product_type, ['Reward Point Offer','Hot Deal'])){
                            $btn .= '
                                        <button class="btn btn-info btn-stroke btn-circle" title="Renew" onclick="renewOffer(' . $row->id . ',\'' . $row->name . '\')"><i class="fa fa-refresh"></i></button>
                                    ';
                        }else{
                            $btn .= '
    ...
    //METHOD TO RENEW PRODUCT STATUS
    public function renewProductOffer(Request $request)
    {
        //COLLECT USER INPUT
        $productId = $request->itemId;
        ...
        //SET NEW STATUS TO "PENDING"
        $product->status = 'pending';

        //RETURN 1 IF STATUS SAVED SUCCESSFULLY
        if($product->save()){
            return response()->json(array('status' => 1));
        }
        //RETURN 0 IF FAILED TO SAVE
        else {
